...
def test_check_modal(browser):
    ...
    assert "Small modal" and "Large modal" in list_name_btn
    assert len_list == 2
    ...
    assert modal_check.btn_close_small_modal.visible()
    time.sleep(2)
    ...
    assert len_body == 574  # Проверка, что открыто окно large-modal
    assert modal_check.body_large_modal.visible()
